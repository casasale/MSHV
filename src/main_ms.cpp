...
{    
    ...
    if (s_mod_ident == 0 || s_mod_ident == 11 || s_mod_ident == 13 || allq65 || jt65emecon)
    {
        ...
        if (id_mshf==2 && s_mod_ident==11) str_t2.append(" SF");
        if (id_mshf==1 && s_mod_ident==11) str_t2.append(" SH");
        str_t.append(str_t2);
    }
...
{
    if ((s_mod_ident == 11 || s_mod_ident == 13 || allq65) && !Multi_answer_mod->isChecked()) ac_Cfm73->setEnabled(true);
...
{   
    ...
    else if (s_mod_ident==11)//ft8 rx to decode  11800 -1700- 13500 -1200- 14700 jt9=50*3456=14.4sec
    {
...
void Main_Ms::SetTxMsgAllTxt(QString str,double tx_freq)
{
    ...
    if (tx_freq==-10.0)//ID QSY = -10.0
        TAllTxt->SetTxt(utc_t.toString("yyyy_MM_dd"),"QSY "+freq_g+" "+ModeStr(s_mod_ident)+mshf+
                        utc_t.toString("|hhmmss|")+str);
    else
        TAllTxt->SetTxt(utc_t.toString("yyyy_MM_dd"),"TX "+freq_g+" "+ModeStr(s_mod_ident)+mshf+
                        utc_t.toString("|hhmmss|")+str);
        ...
        if (s_mod_ident==11 || s_mod_ident==13)//ft8 ft4
        {
            ...
            else
            {
                ...
                for (int i = 0; i < cl; ++i)
                {
                    ...
                    ltxx.clear();
                    nslt++; 
                    if (s_mod_ident==11 && id_mshf != 2) itx_freq+=60;//2.76sf 1.70 60Hz interval
                    else if (s_mod_ident==13)itx_freq+=100; //1.70 100Hz interval
                }
            }
        }
...
void Main_Ms::FileOpen(QString fileName)
{
    ...
    if (time_t.count()>5 && s_mod_ident == 11)
    {
