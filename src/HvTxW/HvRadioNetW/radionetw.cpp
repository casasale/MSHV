...
void HvTcpClient::readTelnet()
{
    ...
    //itcptxt= "callsign:";
    //qDebug()<<ba.data(); qDebug()<<"------------- RX MSG END ---------------";    
    ...
    {
    	writeData(s_login_name); //qDebug()<<"   --MSHV SEND login Call-->"<<s_login_name;
    	stat_disp = 1;
   	} 	
   	if (itcptxt.contains(rx_passw))//if (itcptxt.contains("password",Qt::CaseInsensitive) && itcptxt.contains(":"))
    {
    	writeData(s_pass); //qDebug()<<"   --MSHV SEND Password-->"<<s_pass;
    	stat_disp = 2;
   	}
...
SotpAD;
static SotpAD otp_all_data[MAXOTPCHECK+5];
static QString otp_formated_msg[MAXOTPCHECK+5];
//#define _TEST_OTP_
